...
function getResult(N, K, A, B, vertexList) {
  ...
  while (!visited[B]) {
    ...
    if (!nowVertex || !vertexList.has(nowVertex)) {
      return "-1";
    }

    let neighboringVertices = Object.keys(vertexList.get(nowVertex));

    neighboringVertices.forEach((i) => {
      var vertexNieghborDists = vertexList.get(nowVertex);

      var distantion = dist[nowVertex] + vertexNieghborDists[i];
      if (vertexNieghborDists?.[i] !== undefined && distantion <= dist[i]) {
        dist[i] = distantion;
        prev[i] = nowVertex;
        priorityQueue.enqueue(i, +distantion);
      }
    });
  }
...
function parseData(strings) {
  ...
  vertexList.forEach(([A, B, distAB]) => {
    let startVertexMap = map.get(A);
    if (!startVertexMap) {
      startVertexMap = [];
      map.set(A, startVertexMap);
    }
    startVertexMap.push([B, distAB]);

    let endVertexMap = map.get(B);
    if (!endVertexMap) {
      endVertexMap = [];
      map.set(B, endVertexMap);
    }
    endVertexMap.push([A, distAB]);
  });
