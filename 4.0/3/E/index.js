...
var parsedData = parseData(strings);
...
function getResult(N, driversInfo, roadsMap) {
  ...
  var time = [0, 0];
  for (let i = 2; i <= N; i++) {
    let timeWithoutTransfers = driversInfo[i][0] + d[1][i] / driversInfo[i][1];
    time.push(timeWithoutTransfers);
    priorityQueue.enqueue(i, timeWithoutTransfers);
  }
  ...
  while (priorityQueue.size() !== 0) {
    ...
    if (!visited[nowVertex]) {
      ...
      // let d = fromIToOthers(nowVertex);
      // console.log("+", nowVertex, d);
      visited.forEach((v, k) => {
        if (k !== 0 && v !== true && k !== nowVertex) {
          let totalTime =
            driversInfo[k][0] +
            d[nowVertex][k] / driversInfo[k][1] +
            time[nowVertex];
          ...
          if (totalTime < time[k]) {
            ...
            priorityQueue.enqueue(k, totalTime);
          }
        }
      });
    } else {
  ...
  return [time[nowVertex], way.join(" ")].join("\n");
