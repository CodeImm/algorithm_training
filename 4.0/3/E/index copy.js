...
function getResult(N, driversInfo, roadsMap) {
  console.log(N, driversInfo, roadsMap);

  var vertexMatrix = [];

  for (let i = 0; i <= N; i++) {
    vertexMatrix.push(new Array(N + 1).fill(0));
  }
  ...
  for (let i = 1; i <= N; i++) {
    ...
    while (q.length !== 0) {
      ...
      Object.keys(roadsMap.get(v)).forEach((j) => {
        // console.log(v, ":", i, j);

        if (!visited[j]) {
          let dist = d[v] + roadsMap.get(v)[j];
          d[j] = dist;
          vertexMatrix[i][j] = dist;
  ...
  // для каждой вершины алгоритм дейкстры
  // console.log({ vertexMatrix });
  var resultTime = 0;
  var resultWay = null;
  ...
  for (let i = 1; i <= N; i++) {
    ...
    let timei1 = driversInfo[i][0] + vertexMatrix[i][1] / driversInfo[i][1];
    time[i] = timei1;

    priorityQueue.enqueue(i, timei1);

    let nowVertex = null;

    while (priorityQueue.size() !== 0) {
      nowVertex = +priorityQueue.peek();
      ...
      if (!visited[nowVertex]) {
        ...
        for (let i = 2; i <= N; i++) {
          if (i !== nowVertex) {
            let totalTimeFromNowToIWith =
              driversInfo[nowVertex][0] +
              vertexMatrix[nowVertex][i] / driversInfo[nowVertex][1] +
              driversInfo[i][0] +
              vertexMatrix[i][1] / driversInfo[i][1];
            // console.log(nowVertex, i, totalTimeFromNowToIWith, time[nowVertex]);
            if (totalTimeFromNowToIWith < time[nowVertex]) {
              time[i] = totalTimeFromNowToIWith;
              prev[nowVertex] = i;
              priorityQueue.enqueue(i, totalTimeFromNowToIWith);
            }
          }
        }
      } else {
  ...
  return [resultTime, resultWay.join(" ")].join("\n");
}

function parseData(strings) {
  var N = +strings[0];

  var driversInfo = strings
    .splice(1, N)
    .map((str) => str.split(" ").map(Number));

  driversInfo.unshift(0);

  var roadsInfo = strings
    .splice(1, N - 1)
    .map((str) => str.split(" ").map(Number));

  var roadsMap = new Map();

  roadsInfo.forEach(([A, B, distAB]) => {
    if (!roadsMap.get(A)) {
      roadsMap.set(A, {});
    }
    roadsMap.set(A, { ...roadsMap.get(A), [B]: distAB });

    if (!roadsMap.get(B)) {
      roadsMap.set(B, {});
    }

    roadsMap.set(B, { ...roadsMap.get(B), [A]: distAB });
  });

  return [N, driversInfo, roadsMap];
}
