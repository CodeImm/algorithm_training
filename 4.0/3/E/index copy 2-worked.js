...
function getResult(N, driversInfo, roadsMap) {
      ...
      Object.keys(roadsMap.get(v)).forEach((j) => {
        if (!visited[j]) {
          ...
          if (vertexMatrix[i][j] === 0) {
            vertexMatrix[i][j] = driversInfo[i][0] + dist / driversInfo[i][1];
          }
  ...
  for (let i = 1; i <= N; i++) {
    ...
    while (!visited[1]) {
      ...
      if (!visited[nowVertex]) {
        ...
        for (let k = 1; k <= N; k++) {
          let totalTime = time[nowVertex] + vertexMatrix[nowVertex][k];
  ...
  return [resultTime, resultWay.reverse().join(" ")].join("\n");
}
